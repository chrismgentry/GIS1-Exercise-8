...
With the hillshade created, reorder the layer in your table of content to place the hillshade layer below the DEM. In the symbology options (Raster Layer > Appearance) for the DEM, select an appropriate color scheme and make the DEM layer slightly transparent (play around with varying percentages) to proivde a three-dimensional look to Montromgery County.
...
Right-click on the flood prone cropland dataset and go to _Create Chart > Bar Chart_. On the chart properties pane select _class_name_ as the Category, no aggregation, and add _area_ as the numeric field. To make the chart more presentable, sort the values by descending order on the Y-axis and navigate through the various options (series, axes, guides, format, general) and make any necessary changes to the graph. Keep in mind that the area values in the attribute table are in square feet. To convert this information to acres divide the area values (in ft^2^) by 43560.
